/**
 * Runtime shape validator for the catalog index contract (`./types.ts`).
 *
 * Plain TypeScript, no schema library: this is consumed both by the runtime
 * loader (an untrusted-at-runtime JSON asset) and by the index generator's own
 * self-check, so it must never throw — it always returns a discriminated
 * result describing exactly what is wrong when the input is invalid.
 */
...
const INSTALL_SCOPES: InstallCommandScope[] = [
...
/** Pushes a problem prefixed with the path it was found at. */
function fail(problems: string[], path: string, message: string): void {
...
function validateInstallCommand(
  ...
  if (!isString(value.scope) || !INSTALL_SCOPES.includes(value.scope as InstallCommandScope)) {
    fail(
      problems,
      `${path}.scope`,
      `must be one of ${INSTALL_SCOPES.join(", ")}`,
    );
    ok = false;
  }
  return ok;
...
function validateArtifact(
  ...
  if (!isString(value.kind) || !ARTIFACT_KINDS.includes(value.kind as ArtifactKind)) {
  ...
  if (
    value.documentationExcerpt !== undefined &&
    !isString(value.documentationExcerpt)
  ) {
    fail(
      problems,
      `${path}.documentationExcerpt`,
...
/**
 * Validates that `value` structurally matches the `CatalogIndex` contract.
 * Never throws: on failure returns every problem found (not just the first),
 * with a `path`-prefixed human-readable message per problem.
 */
export function validateCatalogIndex(value: unknown): ValidationResult {
  ...
  if (
    value.marketplaceDescription !== undefined &&
    !isString(value.marketplaceDescription)
  ) {
  ...
  return { valid: true, data: value as unknown as CatalogIndex };
}
